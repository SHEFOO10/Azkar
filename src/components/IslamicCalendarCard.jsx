...
const IslamicCalendarCard = ({ title, description, date, highlight }) => {
  return (
    <div className="bg-card rounded-xl p-6 shadow-md w-full max-w-2xl">
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-2">
          <FaRegStar className="text-primary-dark dark:text-primary-light text-xl" />
          <h2 className="text-lg font-semibold not-dark:text-gray-900">{title}</h2>
        </div>
        <span className="text-primary-dark dark:text-primary-light font-medium">{date}</span>
      </div>
