...
const DailyAzkarSection = ({ title, icon, colorOne, colorTwo, azkar: _azkar }) => {
  ...
  return (
    <div className="w-full max-w-md h-fit dark:bg-gray-800 shadow-lg rounded-2xl overflow-hidden" dir={isArabic ? 'rtl' : "ltr"}>
      <div
      ...
      <div className={`flex flex-col ${azkar.length === 0 ? "" : 'p-4'} gap-4`}>
        {azkar.map((item, index) => (
          <div key={index} className="cursor-pointer dark:bg-gray-700 dark:text-white p-4 rounded-xl shadow-sm transition hover:scale-105" onClick={() => handleRepeation(index)}>
